        ...
        {data.list.slice(0,7).reverse().map((item, index) => (
         ...
         {item.weather && item.weather[0].main==="Clear"?
          ...
          <img src={Rain} alt="Rain"/>:null}
                ...
                <p className="description" style={item.weather &&( item.weather[0].description==="broken clouds") ? {marginLeft: "-2.5vw"}:( item.weather[0].description==="scattered clouds")?{marginLeft: "-2.6vw"}:( item.weather[0].description==="few clouds")?{marginLeft: "1.3vw"}:( item.weather[0].description==="overcast clouds")?{marginLeft: "-3.2vw"}:( item.weather[0].description==="moderate rain")?{marginLeft: "-2.7vw"}:( item.weather[0].description==="light rain")?{marginLeft: "0.4vw"} :null}> {item.weather[0].description}</p>
